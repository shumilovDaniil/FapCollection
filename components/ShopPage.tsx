...
const ShopPage: React.FC<ShopPageProps> = ({ currencies, setCurrencies }) => {
  ...
  return (
    <div className="animate-fade-in">
      <h2 className="text-3xl font-heading mb-8 text-[color:var(--brand-accent)] text-center">Магазин</h2>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
        {shopItems.map((item, index) => (
          <div key={index} className="bg-[color:var(--brand-panel)] border border-[color:var(--brand-accent)]/50 p-6 flex flex-col text-center">
            <h3 className="text-3xl font-heading text-[color:var(--brand-accent)] mb-2">{item.name}</h3>
            <div className="my-4 flex-grow flex flex-col items-center justify-center">
                <div className="flex items-center text-4xl font-black font-heading">
                   {item.type === 'eddies' ? <EddyIcon className="w-10 h-10 mr-3 text-[color:var(--brand-accent)]" /> : <LustGemIcon className="w-10 h-10 mr-3 text-purple-400" />}
                   <span>{item.amount.toLocaleString()}</span>
                </div>
            </div>
            <button
                onClick={() => handlePurchase(item)}
                className="w-full bg-[color:var(--brand-accent)] hover:bg-[color:var(--brand-warning)] text-black font-bold py-3 px-4 transition-colors flex items-center justify-center space-x-2 text-lg transform active:scale-95"
            >
