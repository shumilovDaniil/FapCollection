...
const ChestComponent: React.FC<{ chest: Chest; onOpen: () => void }> = ({ chest, onOpen }) => {
    return (
        <div className="bg-[color:var(--brand-panel)] border border-[color:var(--brand-teal)]/30 rounded-xl p-6 flex flex-col items-center text-center transform hover:scale-105 hover:border-[color:var(--brand-teal)] transition-all duration-300 shadow-lg shadow-black/30">
            ...
            <button
                onClick={onOpen}
                className="mt-4 w-full bg-[color:var(--brand-orange)] hover:brightness-110 text-[color:var(--brand-bg)] font-bold py-3 px-4 rounded-full transition-all duration-200 flex items-center justify-center space-x-2 text-lg transform active:scale-95 shadow-lg shadow-[color:var(--brand-orange)]/20"
            >
